...
Additionally, we will be including some attention checks, but not filtering on them, because the original study did not filter on them.
...
#### Import data
test <- read_survey('~/Desktop/Test_data.csv')

#### Data exclusion / filtering

test$fail <- (rowSums(test %>% select(fail_ck_1, fail_ck_2)))

#### Prepare data for analysis - create columns etc.

test$harm <- ifelse(test$condition=="lib_exp", 1, 0)

test$orig <- ifelse(test$condition=="orig_con", 1, 0)

test$newcon <- ifelse(test$condition=="env_con", 1, 0)

test$ideology <- scale(test$Pol_ideology)[,1]

test$attitude <- (rowSums(test %>%
...
test_model = lm(scale(attitude)~harm*ideology+orig*ideology+newcon*ideology, data = test)
